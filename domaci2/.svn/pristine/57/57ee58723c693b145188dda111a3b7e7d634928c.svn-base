...
int main(int arc, char **argv)
{
  ...
  static double err_thrd = 0;
  #pragma omp threadprivate(err_thrd)
  static int seed = 123456789;
  #pragma omp threadprivate(seed)

  #pragma omp parallel firstprivate(ni,nj,nk), private(wt,w_exact,steps_ave), \
      reduction(+:err) reduction(+:n_inside)
  {
    ...
    for (int i = 1; i <= ni; i++){
      for (int j = 1; j <= nj; j++){
        for (int k = 1; k <= nk; k++){
            ...
            double x1,x2,x3,ut,us,vt,vs,vh,we,w,dx,dy,dz;
            ...
            err_thrd = err_thrd + pow(w_exact - wt, 2);
            // printf("\nerr_thrd%d=%d", omp_get_thread_num(), err_thrd);
