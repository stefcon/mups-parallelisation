...
int main(int argc, char* argv[])
{
  ...
  int move;
  double x[npart * 3], vh[npart * 3], f[npart * 3], f_master[npart * 3];
  double ekin;
  double vel;
  double sc;
  double start, time;
  ...
  if (rank == MASTER) {
    ...
    for (move = 1; move <= movemx; move++)
    {
      ...
      MPI_Bcast(x, 3*npart, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
      ...
      */
      ...
      MPI_Reduce(f, f_master, npart*3, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
      MPI_Reduce(&epot, &epot_master, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
      MPI_Reduce(&vir, &vir_master, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);

      if (rank == MASTER) {
        epot = epot_master;
        vir = vir_master;
        memcpy(f, f_master, npart*3*sizeof(double));
        /*
