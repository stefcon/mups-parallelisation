...
__device__ float potentialCuda(float x, float y, float z)
{
  return 2.0 * ((x / A / A)*(x / A / A) + (y / B / B)*(y / B / B) + (z / C / C)*(z / C / C)) + 1.0 / A / A + 1.0 / B / B + 1.0 / C / C;
}

__host__ __device__ float r8_uniform_01(int *seed)
{
  int k;
  double r;
...
__global__ void feyman_kernel_float(float *cu_points, float* cu_reduction_mem, int N) {

  extern __shared__ float sdata[];

  int ind = blockIdx.x*4;
  int tid = threadIdx.x;
  float x1 = cu_points[ind];
  float x2 = cu_points[ind + 1];
  float x3 = cu_points[ind + 2];
  float w_exact = cu_points[ind + 3];

  int seed = 123456789 - blockIdx.x * threadIdx.x;

  float vs,vh,w,dx,dy,dz,chk,us, ut;
  bool x_r, y_r, z_r;
...
float testCuda(int arc, char **argv)
{
  float a = 3.0;
  float b = 2.0;
  float c = 1.0;
  ...
  float * points_to_calc = (float *)malloc(4*ni*nj*nk*sizeof(float));
  int ind = 0;
  ...
  int pointsSize = 4*n_inside*sizeof(float);
  points_to_calc = (float*) realloc(points_to_calc, pointsSize);

  int reduceSize = n_inside*sizeof(float);
  float* reduction_mem = (float*)malloc(reduceSize);

  // Compute number of blocks needed based on array size and desired block size
  int numBlocks = n_inside;
  int sharedMemSize = NUM_OF_THREADS*sizeof(float);
  
  float *cu_points, *cu_reduction_mem;
  ...
  feyman_kernel_float<<< dimGrid, dimBlock, sharedMemSize >>>( cu_points, cu_reduction_mem, N);
